...
class UssdSession:
    ...
    def _process_choice(self, choice: int) -> None:
        options_count = len(self.current_menu.get_options())
        back_option = options_count + 1 if self.current_menu.get_parent() is not None else -1
        quit_option = options_count + 1 if back_option == -1 else options_count + 2
        
        if choice == quit_option:
            print("Fin de la session USSD.")
            sys.exit(0)
        elif choice == back_option and back_option != -1:
